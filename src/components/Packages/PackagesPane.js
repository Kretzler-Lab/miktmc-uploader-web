...
class PackagesPane extends Component {
    ...
    render() {
        ...
        let lockStateOptions = [
            {value: "UPLOAD_LOCKED", label: "Locked"},
            {value: "UPLOAD_SUCCEEDED", label: "Unlocked"},
        ];
        return (
    			...
    			<header id="packages-filter-controls" className="fixed-top-subnav pt-3">
					<div className='container'>
						<Row noGutters>
                            ...
                            <Col xs={12} md={"auto"} className="mx-sm-auto ml-md-0 mr-md-1">
								<FilterControl className="filter-control" placeholder="Filter by lock state" defaultFilter={lockStateOptions[1]} options={lockStateOptions} type={filterActions.filterTypes.UPLOAD_STATE} addFilter={this.props.addFilter} removeFilter={this.props.removeFilter}/>
							</Col>
