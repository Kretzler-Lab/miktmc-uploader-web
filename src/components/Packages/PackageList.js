...
class PackageList extends Component {
    ...
    render() {
        ...
        if (this.state.unfilteredPackages.length === 0 || Object.keys(this.props.stateDisplayMap).length === 0) {
            ...
            panels = this.state.packages.map((uploadPackage, index) => {
                return <PackagePanelContainer key={index} index={index} uploadPackage={uploadPackage} getPackages={this.getPackages.bind(this)}/>;
            });
        }
